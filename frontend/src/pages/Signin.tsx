...
function Form() {
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
        const response = await AxiosInstance.put('user', {username, password})
        console.log(response.data)
    } catch (error) {
        console.error('an error occured on signin', error)
    }
  }
  return(
    <form onSubmit={handleSubmit} className="max-w-sm mx-auto border rounded-lg p-2 mb-6">
      <div className="mb-5">
            ...
            <button type="submit"  className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center cursor-pointer dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Signin</button>
    </form>
  )
}
...
function Login() {
  return(
      <>
          <a href='/login'>Already a user?<span className='text-blue-500 hover:text-blue-700 cursor-pointer'> Login</span></a>
      </>
  )
}